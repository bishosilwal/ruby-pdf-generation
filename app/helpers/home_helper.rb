module HomeHelper
  ...
  def create_document
    ...
    pdfs.each do |pdf|
      ...
      if prev_doc.any?
        flash[:alert]="#{pdf.original_filename} is already exist .Please choose different file name. Operation aborted!!"
        return
      end 
    end
    @document_model=[]
    ...
    @document_model.last.document=pdf 
    @document_model.last.user_id=current_user.id
     #put newly created document to the root folder
    @document_model.last.folder_id=@parent_folder.id
    end
